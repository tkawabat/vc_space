...
class RoomOfferUserList extends HookConsumerWidget {
  ...
  void Function(int) createFetchFunction(UserSearchInputEntity searchUser) {
    return (int pageKey) {
      FunctionModel().searchUser(pageKey, searchUser).then((list) {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final fetchFunction = useState(createFetchFunction(searchUser));

    useEffect(
      () {
        _pagingController.removePageRequestListener(fetchFunction.value);
        fetchFunction.value = createFetchFunction(searchUser);
  ...
  Widget buildUserCard(UserSearchEntity userSearch) {
    if (room != null && RoomService().isJoined(room!, userSearch.user.uid)) {
    ...
    return UserCard(
      ...
      trailingButtonText: room != null &&
              RoomService().getRoomUser(room!, userSearch.user.uid) == null
          ? '誘う'
          : null,
